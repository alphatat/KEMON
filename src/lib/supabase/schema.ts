...
//      .notNull()
    ...
    sourceUrls: jsonb("source_urls").$type<{ site: string; url: string }[]>().default([]),
  },
  (table) => {
    return {
...
export const reviews = pgTable(
  ...
  (table) => ({
    ...
    uniqUserProduct: uniqueIndex("unique_user_product_review").on(table.profileId, table.productId),
  })
);
...
export const reviewLikes = pgTable(
  ...
  (table) => ({
    pk: primaryKey({ name: "review_likes_pk", columns: [table.profileId, table.reviewId] }),
  })
);
